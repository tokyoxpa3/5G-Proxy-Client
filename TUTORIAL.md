...
- **📲 5G 行動 IP: `49.215.85.112`** ← 驗證用：所有經代理出去的流量，出口 IP 都應等於它
...
驗證原理：Server 端「5G 行動 IP」= 49.215.85.112，任何**經代理出去的流量**，出口 IP 都應等於它。
...
```text
49.215.85.112        <- 與 Server 端「5G 行動 IP」完全一致（成功）
```
...
```text
49.215.85.112        <- Client 手機的出口 IP = Server 的 5G IP，證明流量全部走隧道（成功）
```
