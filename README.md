...
- **自動重連**：60 秒內 ≥3 次網路層連線失敗即判定伺服器斷線，以遞增延遲（10s→上限 5 分鐘）自動重啟隧道；任一連線成功即重置（認證被拒等協定層錯誤不觸發）。自動重連採**軟重連**：不拆 TUN，僅重置引擎連線狀態避免洩漏真實 IP；並在重連前經 protect() 過的 UDP socket 繞過隧道重新解析伺服器 hostname（DDNS／IP 變動可自動追蹤）
- **流量統計**：常駐通知即時顯示 ↑/↓ bytes 與 TCP/UDP session 數（每 3 秒更新）
- **IPv4 + IPv6 雙棧**：TUN 同時配置 `10.8.0.2/32` 與 `fd00::2/128`，UDP/TCP 皆支援 v6
- **Per-App 模式**：三種模式任選
  - 🌐 全局（所有流量走隧道）
  - 🎯 白名單（僅勾選的 App 走隧道，其餘走本機網路）
  - 🚫 排除（勾選的 App 走本機網路，其餘走隧道）
  - App 清單內建**搜尋框**、可切換**顯示系統 App**、列內附套件名
- **自訂 DNS**：可指定主要/次要 DNS 伺服器（預設 8.8.8.8 / 1.1.1.1）
- **開機自啟**：勾選「開機自動啟動隧道」後，開機即自動以最後設定拉起隧道
- **Quick Settings 磁貼**：快速面板一鍵開關隧道
- **多組設定檔**：可儲存 / 載入 / 刪除多組伺服器設定，並支援**匯出 / 匯入**（剪貼簿 JSON 往返）
- **通知操作**：通知列附「重啟」「停止」快捷按鈕
- **本機 ICMP Echo 回應**：隧道內也能直接 ping 通（IPv4 / IPv6）
...
完整圖文操作教學（實機截圖 + 架構圖 + 驗證步驟，含 5G-Proxy-Pro 伺服器端設定）：
